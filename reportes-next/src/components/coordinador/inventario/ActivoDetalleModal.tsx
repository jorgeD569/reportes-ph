...
import { formatFechaAR, formatTimestamptzDiaAR } from '@/lib/date'
...
/** fecha_desde / fecha_hasta (composición) son timestamptz → día operativo AR. */
...
export function ActivoDetalleModal({
  ...
  const subtitle = activo
    ? [
        activo.codigo_interno ? `Código ${activo.codigo_interno}` : null,
        activo.numero_serie ? `Serie ${activo.numero_serie}` : null,
        activo.descripcion,
      ]
        .filter(Boolean)
        .join(' · ')
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={esConjunto ? 'Detalle del conjunto' : 'Detalle del activo'}
      subtitle={subtitle}
      compact
      ...
      {!error && activo ? (
        <div className="space-y-6">
          ...
          <section>
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              ...
              <DataField
                ...
                value={displayCodigoInterno(activo.codigo_interno)}
              />
              ...
              {esConjunto ? (
                <div className="sm:col-span-2">
                  <DataField
                    ...
                    value={displayUbicacionConjunto(
                      ubicacionVisibleConjunto(activo),
                    )}
                  />
                </div>
              ) : (
                <>
                  <DataField
                    label="Ubicación propia"
                    value={display(activo.ubicacion)}
                  />
                  <DataField
              ...
              <div className="sm:col-span-2">
                ...
                >
                  {displayObservaciones(activo.observaciones)}
                </div>
              </div>
            </div>
          </section>

          <ActivoAdjuntosGallery adjuntos={adjuntos} />

          {esConjunto ? (
            <section className="space-y-3">
              <div>
                <div className={COORD_SECTION_TITLE}>Componentes instalados</div>
              ...
              {componentes.length === 0 ? (
                <EmptyState
                  ...
                  description="Este conjunto aún no tiene componentes asignados."
                />
              ) : (
              ...
              <details
                ...
                open={histCompOpen}
                onToggle={(e) =>
                  setHistCompOpen((e.target as HTMLDetailsElement).open)
                }
              >
          ...
          {pertenece && pertenenciaActual ? (
            <section className="space-y-3">
              <div className={COORD_SECTION_TITLE}>Pertenece a un conjunto</div>
              ...
              <details
                ...
                open={histManOpen}
                onToggle={(e) =>
                  setHistManOpen((e.target as HTMLDetailsElement).open)
                }
              >
                <summary className="cursor-pointer px-4 py-3 text-sm font-semibold text-white">
                  Historial de conjuntos anteriores ({histMan.length})
                </summary>
                <div className="space-y-2 border-t border-slate-700 px-4 py-3">
                  {histMan.length === 0 ? (
                    <p className={cn('text-sm', COORD_TEXT_MUTED)}>Sin historial.</p>
                    ...
                    histMan.map((h) => (
                      ...
                      >
                        <button
                          ...
                          onClick={() =>
                            goToActivo(h.manifold?.id ?? h.conjunto_id)
                          }
                        >
